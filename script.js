...
var points = textToPoints("HAPPY 2 0 2 0", 40, "Anton");
var names = textToPoints("鲍瑞", 50, "serif");
...
fireworks.push(new Firework(width / 2, height, 0, -9.5, 10, "gold", true, true));
setInterval(function() {
  fireworks.push(new Firework(width / 2, height, 0, -9.5, 10, "gold", false, false));
}, 5000);
...
function Firework(x, y, vx, vy, radius = 5, color = "white", title = false, name = false) {
  this.x = x;
  this.y = y;
  this.vx = vx;
  this.vy = vy;
    ...
    if (this.vy >= 0) {
      ...
      if (this.name) {
        ...
        for (let i = 0; i < names.length; i++) {
          let p = names[i];
          let v = {
            x: (p.x ) * scale + (Math.random() - 0.5) * 0.1,
            y: (p.y - 100) * scale + (Math.random() - 0.5) * 0.1
          };
